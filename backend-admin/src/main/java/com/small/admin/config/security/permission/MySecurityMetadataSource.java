package com.small.admin.config.security.permission;
...
public class MySecurityMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public void loadResourceDefine(){
        ...
        List<Permission> permissions = permissionService.lambdaQuery()
                .eq(Permission::getType, CommonConstant.PERMISSION_OPERATION)
                .eq(Permission::getStatus, StatusEnum.NORMAL.getValue())
                .orderByAsc(Permission::getSortOrder).list();

        for(Permission permission : permissions) {
            if(StrUtil.isNotBlank(permission.getId())&&StrUtil.isNotBlank(permission.getPath())){
                configAttributes = new ArrayList<>();
                cfg = new SecurityConfig(permission.getId());
                //作为MyAccessDecisionManager类的decide的第三个参数
                configAttributes.add(cfg);
                //用权限的path作为map的key，用ConfigAttribute的集合作为value
                map.put(permission.getActionMethod()+permission.getPath(), configAttributes);
            }
        }
    }
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) throws IllegalArgumentException {
        ...
        String url_key = method + url;

//        log.info("user request url: {}, method: {}", url, method);
        PathMatcher pathMatcher = new AntPathMatcher();
        Iterator<String> iterator = map.keySet().iterator();
        while (iterator.hasNext()) {
            String resURL = iterator.next();
//            log.info("resURL: {}",resURL);
            if (StrUtil.isNotBlank(resURL)&&pathMatcher.match(resURL,url_key)) {
